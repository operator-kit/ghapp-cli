...
func archiveName(version string) string {
	...
	return fmt.Sprintf("ghapp_%s_%s_%s.%s", version, runtime.GOOS, runtime.GOARCH, ext)
}
...
// Update downloads and installs the release, replacing the current binaries.
func Update(release *ReleaseResponse, out io.Writer) error {
	...
	// Extract to temp dir
	tmpDir, err := os.MkdirTemp("", "ghapp-update-*")
	if err != nil {
	...
	// Replace ghapp-gh if it exists in the install dir
	ghName := "ghapp-gh"
	if runtime.GOOS == "windows" {
		ghName = "ghapp-gh.exe"
	}
	ghDst := filepath.Join(installDir, ghName)
	if _, err := os.Stat(ghDst); err == nil {
		ghSrc := filepath.Join(tmpDir, ghName)
		if _, err := os.Stat(ghSrc); err == nil {
			fmt.Fprintf(out, "Replacing %s...\n", ghDst)
			if err := replaceBinary(ghSrc, ghDst); err != nil {
				fmt.Fprintf(out, "Warning: failed to replace %s: %v\n", ghName, err)
			}
		}
	}
